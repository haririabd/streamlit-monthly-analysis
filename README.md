...
![Architecture Diagram](docs/Hybrid_Automation_Workflow-min.png)
...
1.  **Ingestion:** Automation flow via RPA (Power Automate) read email and saves vendor notifications in text file to a local `Input` directory.
2.  **Processing:** `outage_processor.py` scans the local `Input` directory for new files, applies Regex extraction, and upserts data into `outage_master.db`.
3.  **Visualization:** Streamlit reads the local database to display live outage statistics.

## Setup & Configuration
1.  **Database Path:** Configure the `DB_PATH` variable in `app.py` and `outage_processor.py` to point to local data directory
2.  **Regex Patterns:** If adapting for a new vendor, update the regex logic in `outage_processor.py` function `parse_incident_email()`.

## Quick overview
- Streamlit app:
  - [app.py](app.py) — main dashboard using Altair charts and tables.
- Data pipeline helpers:
  - Data loader: [`utils.data_loader.load_excel`](utils/data_loader.py) or [`utils.data_loader.load_data`](utils/data_loader.py)
  - Cleaning: [`utils.data_cleaner.clean_tt_data`](utils/data_cleaner.py) or [`utils.data_cleaner.clean_db_data`](utils/data_cleaner.py)
...
- See [requirements.txt](requirements.txt)
  - streamlit
  - pandas
  - openpyxl (for Excel support, can ignore if using SQLite pipeline)
  - altair
  - python-dotenv
  - fpdf

### Data Ingestion
- Default Excel file (previous version): source/sourcefile.xlsx (loaded by [`utils.data_loader.load_excel`](utils/data_loader.py))
- SQLite Database (New version): `outage_master.db` located at `DB_PATH` (loaded by [`utils.data_loader.load_data`](utils/data_loader.py))
- Data is cleaned and normalized by [`utils.data_cleaner.clean_tt_data`](utils/data_cleaner.py) for Excel pipeline or by [`utils.data_cleaner.clean_db_data`](utils/data_cleaner.py) for SQLite pipeline. The cleaner will:
...
- The loader function ```utils.data_loader.load_excel``` looks in the ```source/``` directory, ensure the Excel source file is there.
